...
'''
Train a ConvLSTM model with MLP for regression
（no timestep set, if timestep >=2 is wanted, check 'maskrcnn_torch.py'）
(Optional: test on the test dataset)
'''
...
class ImageRegressionLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_mlp_hidden_units, num_regression_output):
        ...
        self.fc = nn.Linear(num_mlp_hidden_units, num_regression_output) 
...
# Define dataset (Input: images, no timestep set, normalized and scaled)
class CustomDataset(Dataset):
    def __init__(self, image_folder, targets_file, transform=None, target_scale_factor=100.0):
    ...
    def __getitem__(self, idx):
        ...
        target = self.targets[idx] * self.target_scale_factor 
        return image, target
...
# Load dataset
train_images_folder = 'C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset_sqcrop/train/'
train_targets_file = 'C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset_sqcrop/train/norm_targets.txt'
test_images_folder = 'C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset_sqcrop/test/'
test_targets_file = 'C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset_sqcrop/test/norm_targets.txt'
...
# Training
# Initialize model
num_regression_output = 3  # Number of regression output num (x, y, z)
input_size = 3 
hidden_size = 128
num_layers = 2
...
# Save trained model
torch.save(model.state_dict(), 'conv_lstm_mlp_reg.pth')
...
#         denormalized_targets = batch_targets / 100.0  # sacle back
