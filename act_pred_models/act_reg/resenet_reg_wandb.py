...
wandb.init(
    ...
    project="amiga-IL-test-resnet-regression",
    # name="0.001-10-32"
...
targets_train = read_targets_from_file('C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset/train/targets.txt')
...
# Load dataset
image_folder = ('C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset/train/')
...
# Test the model:
targets_test = read_targets_from_file('C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset/test/targets.txt')
...
image_test_folder = ('C:/D/Imperial/Thesis/amiga_dataset/IL/act_pred_dataset/test/')
...
with torch.no_grad():
    ...
    for batch_images, batch_targets in test_loader:
        ...
        for i in range(predictions.size(0)):
            ...
            running_mae += mean_absolute_error(predictions, batch_targets).item()
            running_rmse += root_mean_squared_error(predictions, batch_targets).item()
            epoch_test_mae = running_mae / len(train_loader)
            epoch_test_mae.append(epoch_test_mae)
            epoch_test_rmse = running_rmse / len(train_loader)
            epoch_test_mae.append(epoch_test_rmse)
            wandb.log({"test_mae":  epoch_test_mae, "test_rmse":  epoch_test_rmse})
